...
We use the Sigmoid function because it's a **differentiable function** (a piecewise linear function, given 0 value if x < 0.5 and 1 in another case, is not a differentiable one).
...
$\ell(y, \hat{y}) = -(ylog(\hat{y}) + (y +1)log(1 - \hat{y})$
...
Then, the $\frac{1}{n}$ constant dissapear. 
